...
{
    public class T89OpCode
    {
        public const int T89OP_SIZE = 2;
        public T89OpCode LastOpCode { get; private set; }
        public T89OpCode NextOpCode { get; private set; }
        ...
        public T89OpCode(ScriptOpCode op_info)
        {
            Code = op_info;
        }

        protected T89OpCode() { } //allows derived opcodes to not have to call base

        public virtual void Link(T89OpCode lastOp, T89OpCode nextOp)
        {
            LastOpCode = lastOp;
            NextOpCode = nextOp;
        }

        public T89OpCode GetEndOfChain()
        {
            if (NextOpCode == null)
                return this;
        ...
        public virtual void Append(T89OpCode NextOp)
        {
            if (NextOp == null)
                return;
            NextOpCode = NextOp;
        ...
        public virtual void Insert(T89OpCode Target)
        {
            if (Target == null)
                return;
        ...
        public void Commit(ref List<byte> data, ref uint BaseAddress, T89ScriptMetadata EmissionTable)
        {
        ...
        {
            return T89OP_SIZE; //All base opcodes are only 2 bytes
        }

        public virtual uint GetCommitDataAddress()
        {
            return CommitAddress + T89OP_SIZE; //Base opcodes have no data
        }
    ...
    public class ScriptOpMetadata
    {
        ...
        public static readonly ScriptOpMetadata[] OpInfo =
        {
            ...
            new ScriptOpMetadata(ScriptOpCode.WaitTill,                         ScriptOpType.Notification,      ScriptOperandType.UInt8),
            new ScriptOpMetadata(ScriptOpCode.Notify,                           ScriptOpType.Notification,      ScriptOperandType.None),
            new ScriptOpMetadata(ScriptOpCode.EndOn,                            ScriptOpType.Notification,      ScriptOperandType.UInt8),
            ...
            new ScriptOpMetadata(ScriptOpCode.SetLocalVariableCached,           ScriptOpType.SetVariable,       ScriptOperandType.UInt8),
            ...
            new ScriptOpMetadata(ScriptOpCode.GetGlobalObjectRef,               ScriptOpType.StackPush,         ScriptOperandType.UInt8),
            new ScriptOpMetadata(ScriptOpCode.EvalGlobalObjectFieldVariable,    ScriptOpType.Variable,          ScriptOperandType.GlobalFieldVariable),
            new ScriptOpMetadata(ScriptOpCode.WaitFrame,                        ScriptOpType.Call,              ScriptOperandType.None),
            new ScriptOpMetadata(ScriptOpCode.CastAndEvalFieldVariable,         ScriptOpType.Variable,          ScriptOperandType.VariableName),
            new ScriptOpMetadata(ScriptOpCode.FirstArrayKeyCached,              ScriptOpType.Call,              ScriptOperandType.UInt8),
            new ScriptOpMetadata(ScriptOpCode.SetNextArrayKeyCached,            ScriptOpType.SetVariable,       ScriptOperandType.UInt8),
